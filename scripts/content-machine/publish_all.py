...
Cross-publish an article to all platforms at once:
  - Nostr (Habla.news) — always (no registration needed)
...
def main():
    ...
    nostr_dir = os.path.join(os.path.dirname(script_dir), "nostr")
    ...
    # 1. Nostr (always)
    print("\n--- Nostr (Habla.news) ---")
    nostr_args = ["--title", args.title, "--file", args.file, "--tags", args.tags]
    if args.summary:
        nostr_args += ["--summary", args.summary]
    results["nostr"] = run_script(os.path.join(nostr_dir, "publish_longform.py"), nostr_args)

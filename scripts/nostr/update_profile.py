#!/usr/bin/env python3
"""
Update Nostr profile (kind 0) with Lightning tipping info.
...
def main():
    ...
    btc_address = env.get("PHOENIX_BTC_ADDRESS", "")
    bolt12 = env.get("PHOENIX_BOLT12", "")

    # Nostr profile metadata (kind 0)
    profile = {
        "name": "avika",
        "display_name": "Avika",
        "about": "iOS & web dev from Indonesia. Building passive income from terminal only.\n\nI run: AI DVM (kind 5050) | Free AI API | Digital products on Shopstr\n\nPowered by Ollama, Nostr & Lightning.\n\nZap me or DM for collabs!",
        "website": "",
        "picture": "",
        "banner": "",
    ...
    print(f"\nProfile updated! View at: https://njump.me/{env.get('NOSTR_NPUB', '')}")
    print(f"  Lightning address: {profile.get('lud16', 'not set')}")
